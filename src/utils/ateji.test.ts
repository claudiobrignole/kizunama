...
import type { AtejiCombo } from './ateji';

function comboHasSingleKanjiSpan(combos: AtejiCombo[], kanji: string, moraKey: string) {
...
describe('generateAtejiCandidates: variable-length mora span regression', () => {
  ...
  it('光 (hikaru, 3-mora kun reading) appears as a single-kanji candidate', async () => {
    const combos = await generateAtejiCandidates(wanakana.toKatakana('hikaru'));
    expect(comboHasSingleKanjiSpan(combos, '光', 'ヒカル')).toBe(true);
  });

  it('勇 (isamu, 3-mora kun reading) appears as a single-kanji candidate', async () => {
    const combos = await generateAtejiCandidates(wanakana.toKatakana('isamu'));
    expect(comboHasSingleKanjiSpan(combos, '勇', 'イサム')).toBe(true);
  });

  it('守 (mamoru, 3-mora kun reading) appears as a single-kanji candidate', async () => {
    const combos = await generateAtejiCandidates(wanakana.toKatakana('mamoru'));
    expect(comboHasSingleKanjiSpan(combos, '守', 'マモル')).toBe(true);
  });

  it('excludes nanori-only readings from selection', async () => {
    const combos = await generateAtejiCandidates(wanakana.toKatakana('hajime'));
    expect(comboHasSingleKanjiSpan(combos, '一', 'ハジメ')).toBe(false);
    const candidates = await candidatesForReading(['ハ', 'ジ', 'メ']);
    expect(candidates.every((candidate) => candidate.readingType !== ('nanori' as never))).toBe(true);
  });
});

describe('generateAtejiCandidates: general behaviour', () => {
  it('returns fully-covered combinations for an easy short name', async () => {
    const combos = await generateAtejiCandidates('マリ');
    expect(combos.length).toBeGreaterThan(0);
    expect(combos[0].unmatchedMoraCount).toBe(0);
    expect(combos[0].spans.every((s) => s.kanji)).toBe(true);
  });

  it('flags mora with no native kanji reading as unmatched instead of failing', async () => {
    // ヴォ has no kanji on/kun/nanori reading anywhere in the jōyō+jinmeiyō pool.
    const combos = await generateAtejiCandidates('ヴォ');
    expect(combos.length).toBeGreaterThan(0);
    expect(combos[0].unmatchedMoraCount).toBeGreaterThan(0);
    expect(combos[0].spans.some((s) => s.kanji === null)).toBe(true);
  });

  it('ranks boosted/longer-span candidates above plain single-mora kun matches', async () => {
    const combos = await generateAtejiCandidates(wanakana.toKatakana('hikaru'));
  ...
  it('computes per-Kanji and length-weighted combination fit', async () => {
    const combos = await generateAtejiCandidates('マリ', 8, [10, 30]);
    expect(combos[0].spans.map((span) => span.fitPercent)).toEqual([90, 70]);
    expect(combos[0].fitPercent).toBe(80);
  });

  it.each(['マリ', 'ルカ', 'ヒカル'])('snapshots the ranked ateji pipeline for %s', async (name) => {
    const combos = await generateAtejiCandidates(name, 3);
    const summary = combos.map((combo) => ({
  ...
  it('returns an empty array for empty input', async () => {
    expect(await generateAtejiCandidates('')).toEqual([]);
  });
});
